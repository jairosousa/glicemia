...
import { FormularioHumor } from "@/components/FormularioHumor";
import { obterPapelAcesso } from "@/lib/acesso";
import type { Perfil } from "@/lib/perfil";
import { criarClienteServidor } from "@/lib/supabase/server";

export default async function NovoHumorPage({
  params,
}: {
  ...
  return (
    <main className="mx-auto flex w-full max-w-sm flex-col gap-6 px-5 py-8 sm:py-12">
      <div className="flex flex-col gap-1.5">
        ...
        <h1 className="text-2xl font-bold">Registrar humor</h1>
      </div>

      <FormularioHumor perfilId={perfilId} perfilNome={perfil.nome} usuarioId={user.id} />
    </main>
  );
}
